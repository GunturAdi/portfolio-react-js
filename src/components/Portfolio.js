...
const Portfolio = () => {
  ...
  useEffect(() => {
    setPortfolios([
      ...
      {
        title: 'membangun aplikasi pembelian',
        description: 'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Asperiores, porro.',
        tech: 'teknologi yang digunakan',
        imageUrl:
  ...
  return (
    <>
      <Navbar />
      <h1 className="text-2xl font-semibold pl-7 mb-10">Portfolio Saya</h1>
      <div className="container px-10 py-6 mx-auto rounded-lg shadow-lg bg-slate-300 mb-10">
        {portfolios.map((portfolio, index) => {
          return (
            <div
              className="container px-6 font-inter max-w-md mx-auto sm:max-w-xl md:max-w-5xl lg:flex lg:max-w-full lg:p-0"
              key={index}
            >
              <div className="lg:p-12 lg:flex-1">
                <h3 className="text-xl font-bold text-slate-800 ">{portfolio.title}</h3>
                <p className="mt-2 text-slate-500 text-lg">{portfolio.description}</p>
                <p className="text-lg mb-3 mt-3">{portfolio.tech}</p>
                <div className="flex items-center mt-3">
                  {/* Html */}
                  <a
                    href="#"
                    target="blank"
                    className="w-9 h-9 mr-5 rounded-full flex justify-center items-center border text-black hover:bg-rose-600 hover:text-white"
                  >
                    <svg
                      role="img"
                      width="30"
                ...
                <div className="border-4 mt-7"></div>
              </div>
              <div className="lg:flex lg:flex-1">
                <img src={portfolio.imageUrl} alt="" className="mb-16" />
              </div>
            </div>
          );
        })}
